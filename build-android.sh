...
exe_dir=/data/local/tmp/intro_topi
...
for ((i=0; i < ${#platforms[@]}; i++)); do
    ...
    pycmd="${PWD}/intro_topi.py"

    (
	cd $target_dir
	python ${pycmd} --target="${target}" --target-host="${target_host}"

	file ${PWD}/intro_topi.so
    )

    exe_toolchain_dir=${exe_dir}/${toolchain_name}
    adb push ${target_dir} ${exe_dir}
    echo "run on device"
    adb shell "cd ${exe_toolchain_dir} && ./intro_topi \${PWD}/intro_topi.so intro_topi | tee output.txt"
